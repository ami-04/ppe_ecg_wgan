...
# Load data
# Load data and training logic wrapped to avoid multiprocessing issues on macOS
def main():
    # Load data
    ...
    if cuda:
        if torch.cuda.device_count() > 2:
            generator = torch.nn.DataParallel(generator, device_ids=[0, 1, 2]).to(device)
            discriminator = torch.nn.DataParallel(discriminator, device_ids=[0, 1, 2]).to(device)
        else:
    ...
    if args.epoch != 0:
        ...
        pretrained_path = "saved_models/%s/multi_models_%d.pth" % (args.experiment_name, args.epoch)
        ...
        if args.is_eval:
            sample_images(args.experiment_name, val_dataloader, generator, args.epoch, device)
        ...
        for i, batch in enumerate(dataloader):
            ...
            if i % args.ncritic == 0:
                ...
                if args.from_ppg and args.peaks_only:
                    ...
                    loss_sample_opeaks = criterion_samplewise(fake_B_masked_opeaks, opeaks)
                    loss_sample_rpeaks = criterion_samplewise(fake_B_masked_rpeaks, rpeaks)
                    loss_sample = loss_sample_opeaks / opeak_count + rpeak_weight * loss_sample_rpeaks / rpeak_count
                else:
            ...
            # Gradient penalty
            gradient_penalty = compute_gradient_penalty(discriminator, real_B, fake_B.detach(), real_A, patch, device)
            # Adversarial loss
            loss_D0 = -torch.mean(real_validity) + torch.mean(fake_validity) + args.lambda_gp * gradient_penalty
            loss_D = loss_D0
            ...
            writer.add_scalars('losses3', {'sample_loss': loss_sample.item()}, batches_done)
